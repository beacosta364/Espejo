...
public class GestorVentasConDescuento {
    private Connection conexion;

    public GestorVentasConDescuento(Connection conexion) {
    ...
    public void gestionarVentaConDescuento(int codProducto, int cantidad, int codVendedor, int codCliente) {
        try {
            ...
            PreparedStatement totalComprasStmt = conexion.prepareStatement("SELECT SUM(total) FROM venta5 WHERE cod_cliente = ?");
            totalComprasStmt.setInt(1, codCliente);
            ResultSet totalComprasRs = totalComprasStmt.executeQuery();
            totalComprasRs.next();
            double totalComprasCliente = totalComprasRs.getDouble(1);

            double descuento = 0.0;
            if (totalVentas > 5 && totalComprasCliente > 2000000) {
                descuento = 0.12;
            } else if (totalVentas > 5) {
                descuento = 0.05;
            }
